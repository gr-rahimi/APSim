...
for anml in AnmalZoo:
    anml = AnmalZoo.Levenshtein
    if anml == AnmalZoo.ClamAV:
        continue
    if not os.path.exists(str(anml)):
        os.makedirs(str(anml)) #make directory if it does not exist
    else:
        shutil.rmtree(str(anml)) #remove content
        os.makedirs(str(anml))  # make directory if it does not exist
    ...
    for cc_idx, cc in enumerate(ccs):
        ...
        switch_map = cc.draw_switch_box(str(anml)+"/number_"+ str(cc_idx)+"bfs_cost_"+str(bfs_cost), bfs_label_dictionary)
    ...
    utility.draw_matrix(str(anml)+"/heat_map.png",heat_map, [i/256 for i in range(257)])
